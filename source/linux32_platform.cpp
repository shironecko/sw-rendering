...
void Linux32SetupRenderingBuffers(RenderTarget *renderTarget, u32 width, u32 height,
...
int main(int argc, char **argv) {
	...
	Window window =
	    XCreateWindow(display, rootWindow, 0, 0, 600, 600, 0, visualInfo->depth, InputOutput,
	...
	XSelectInput(display, window,
	             ButtonPressMask | KeyPressMask | KeyReleaseMask | StructureNotifyMask);
	...
	Linux32SetupRenderingBuffers(&renderBuffer, windowWidth, windowHeight, platformMemory,
	                             platformMemorySize);
	...
	do {
		...
		while (XPending(display)) {
			...
			case ConfigureNotify: {
				...
				if (xce.width != windowWidth || xce.height != windowHeight) {
					...
					Linux32SetupRenderingBuffers(&renderBuffer, windowWidth, windowHeight,
					                             platformMemory, platformMemorySize);
				}
			} break;

			case ClientMessage: {
				// yeah, this is bizzare, can't do much about it :(
				if (event.xclient.data.l[0] == wmDeleteMessage) continueRunning = false;
		...
		glBindTexture(GL_TEXTURE_2D, renderTexture);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, renderBuffer.texture->width,
		             renderBuffer.texture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
