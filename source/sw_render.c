...
// TODO: sort this out
void Render(RenderTarget *target, u32 renderMode, Model *model, vec4 cameraPosition,
            mat4 modelM, mat4 viewProjectionM, mat4 screenM,
            vec4 sunlightDirection, Color32 sunlightColor, float ambientIntencity,
            mem_pool *pool) {
	Texture *targetTexture = target->texture;
	float *zBuffer = target->zBuffer;

	u8 *initialHiPtr = pool->hiPtr;
	vec4 *vertices = (vec4 *)MemPushBack(pool, model->verticesCount * sizeof(*vertices));
	vec4 *camDirections =
	    (vec4 *)MemPushBack(pool, model->verticesCount * sizeof(*camDirections));
	for (u32 i = 0, e = model->verticesCount; i < e; ++i) {
		vec4 v = modelM * model->vertices[i];
		camDirections[i] = norm_v3(v - cameraPosition);
		...
		for (u32 i = 0; i < facesToDraw; ++i) {
			...
			if (renderMode & (RenderMode::Shaded | RenderMode::Textured)) {
				...
				if (renderMode & RenderMode::Shaded) {
					...
					float diffuse[3];
					for (u32 j = 0; j < 3; ++j)
						diffuse[j] = clamp(dot_v3(norms[j], sunlightDirection), 0, 1.0f);

					vec4 L = -sunlightDirection;
					float specular[3] = {0};
					for (u32 j = 0; j < 3; ++j) {
						if (diffuse[j]) {
							vec4 V = camDirections[face.v[j]];
							vec4 H = norm_v3(V + L);
							specular[j] = pow(dot_v3(H, norms[j]), 32);
						}
					}
