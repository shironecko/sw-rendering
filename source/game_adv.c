...
void *mem_push(struct mem_pool *pool, size_t size) {
	u8 *old_memory = pool->memory;
	pool->memory += size;
	SDL_assert(pool->memory <= pool->memory_end);

	return old_memory;
}

float ui_text_width_fn(nk_handle userdata, float height, const char *str, int len) {
	struct ui_font *font = userdata.ptr;
	float xpos = 0, ypos = 0;
	for (s32 i = 0; i < len; ++i) {
...
b32 game_update(struct game_data *data, struct gl_functions gl_fns, float delta_time) {
	if (data->kb[SDL_SCANCODE_ESCAPE]) return false;

	struct game_state *state = (struct game_state *)data->memory;
	if (!state->initialized) {
		SDL_Log("Initializing game state...");
		state->initialized = true;

		state->memory = (u8 *)data->memory + sizeof(struct game_state);
		state->memory_end = (u8 *)data->memory + data->memory_size;
		SDL_assert(state->memory_end >= state->memory);
