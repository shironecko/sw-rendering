...
inline float Pow(float val, s32 pow) {
	...
	float result = 1.0f;
	for (s32 i = 0; i < pow; ++i) result *= val;
...
float Dot2(Vector2 a, Vector2 b) { return a.x * b.x + a.y * b.y; }
...
Vector4 operator-(Vector4 vec) { return Vector4{-vec.x, -vec.y, -vec.z, -vec.w}; }
...
float Dot3(Vector4 a, Vector4 b) { return a.x * b.x + a.y * b.y + a.z * b.z; }

float Dot4(Vector4 a, Vector4 b) { return a.x * b.x + a.y * b.y + a.z * b.z + a.w * b.w; }
...
Vector4 Row(Matrix4x4 mat, u32 row) { return mat.rows[row]; }

Vector4 Column(Matrix4x4 mat, u32 column) {
	return Vector4{Row(mat, 0).components[column], Row(mat, 1).components[column],
	               Row(mat, 2).components[column], Row(mat, 3).components[column]};
}

Vector4 operator*(Matrix4x4 mat, Vector4 vec) {
	return Vector4{Dot4(vec, Row(mat, 0)), Dot4(vec, Row(mat, 1)), Dot4(vec, Row(mat, 2)),
	               Dot4(vec, Row(mat, 3))};
}
...
static Matrix4x4 IdentityMatrix() {
	return Matrix4x4{1.0f, 0, 0, 0, 0, 1.0f, 0, 0, 0, 0, 1.0f, 0, 0, 0, 0, 1.0f};
}

static Matrix4x4 ScaleMatrix(float scaleX, float scaleY, float scaleZ) {
	return Matrix4x4{scaleX, 0, 0, 0, 0, scaleY, 0, 0, 0, 0, scaleZ, 0, 0, 0, 0, 1};
};
...
static Matrix4x4 ProjectionMatrix(float vfov, float aspect, float clipNear, float clipFar) {
	...
	return Matrix4x4{x, 0, 0, 0, 0, y, 0, 0, 0, 0, zz, zw, 0, 0, -1.0f, 0};
}
...
static Matrix4x4 ScreenSpaceMatrix(u32 width, u32 height) {
	...
	return Matrix4x4{hw, 0,  0,    hw, // + 0.5f,
	                 0,  hh, 0,    hh, // + 0.5f,
	                 0,  0,  1.0f, 0,  0, 0, 0, 1.0f};
}
